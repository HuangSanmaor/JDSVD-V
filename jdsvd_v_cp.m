function varargout = jdsvd_v_cp(A,varargin)
...
%
% T = JDSVD_V_CP(A,K,SIGMA,NAME,VALUE) configures additional options 
...
%
% T = JDSVD_V_CP(A,K,SIGMA,OPTIONS) alternatively configures the additional
...
%
% [T,U,V,RELRES] = JDSVD_V_CP(A,...) also returns a vector of relative 
...
if M==0 || N==0
    ...
    if nargout == 0
        fprintf('\nAlgorithm: the Standard JDSVD_V_CP algorithm;\n');
...
if Options.disp > 1
    ...
    fprintf('Algorithm: the Standard JDSVD_V_CP algorithm;\n');
end
%-----------------------------------------------%
...
m = min(size(V0,2)-1,Options.maxsize-1);
if m > 0
    V(:,1:m) = V0(:,1:m); 
    AV(:,1:m) = Afun(V(:,1:m),'notransp');
    [U(:,1:m),H(1:m,1:m)] = qr(AV(:,1:m),'econ');  
...
while k_conve < k && OuterIter < Options.maxit
    ...
    H(1:m-1,m) = (V(:,m)'*ATU(:,1:m-1))';
    uh = AV(:,m) - U(:,1:m-1)*H(1:m-1,m);
    [U(:,m),H(m,m)] = tools.normalizing(uh); 
    ATU(:,m) = Afun(U(:,m),'transp'); 
    %-----------------------------%
    ...
    search=1;
    while search
        % Compute the eigendecomposition of the small projected matrix.
        ...
        router = ATU(:,1:m)*Ctemp(:,1)-theta*vfound; 
        ...
        if RelResidual <= max([Options.tol,(eps*normAest)/theta])
            % Update the number of converged singular triplets
            k_conve = k_conve + 1;
            T_conve(k_conve,k_conve) = theta;
            %U_conve(:,k_conve) = U(:,1:m)*Ctemp(:,1);
            U_conve(:,k_conve) = U(:,1:m)*Ctemp(:,1); 
    ...
    iexten = StandardExtendIndexI_CP(Theta,targetin,Options,m);
    if isempty(iexten)  
        Vexten = zeros(N,0);
    else
        ...
        Dexten = Dtemp(:,iexten); 
        Vexten = V(:,1:m)*Dexten;
        Rexten = ATU(:,1:m)*Cexten - Vexten*Texten; 
        Resexten = vecnorm(Rexten)*Texten/normAest^2;
        % Determine the indexes of the singular vectors to be finally exploited
        % in the correction equations.
        jexten = StandardExtendIndexII_CP(Resexten,Options); 
    ...
    [t,iter,~,inflag] = tools.InnerSolverPreJDSVD_CP(Afun,N,router,...
        [V_conve(:,1:k_conve) Options.Vconve],vfound,Vexten,targetin,epsilonin); 
    % Update the computational results.
...
if OuterIter >= Options.maxit
    ...
    T_conve(k_conve,k_conve) = theta; 
    V_conve(:,k_conve) = vfound;
    U_conve(:,k_conve) = tools.normalizing(Afun(vfound,'notransp'));
    % Update the computational results.
    CPUTimeseVec(k_conve) = toc; 
    OuterIterVec(k_conve) = OuterIterCounter; 
    InnerIterVec(k_conve) = InnerIterCounter; 
...
function iexten = StandardExtendIndexI_CP(Theta,targetin,Options,m)
% Determine the indexes of the singular vectors to be extended to the
% correction equations for the first time.
if m > 1
    theta = diag(Theta(2:m,2:m));
    iexten = 1+find(abs(theta.^2-targetin^2)./max(theta.^2,1) <= Options.innprectol1);
else
    iexten = [];
end
iexten = iexten';
...
function iexten = StandardExtendIndexII_CP(Resexten,Options)
% Determine the indexes of the singular vectors to be finally exploited
% in the correction equations.
